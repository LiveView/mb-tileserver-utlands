...
tm._config = {
    ...
    geo_data: path.join(__dirname, '..', '/geo_data/'),
    target: path.join(__dirname, '..', '/targets')
};
...
// Update data location in xml to specified location
tm.parser = new xml2js.Parser();
tm.updateXMLFilePath = function(str, loc, callback) {
    tm.parser.parseString(str, function (err, result) {
        if (err) return callback(err);
        for (var l in result.Map.Layer){
            result.Map.Layer[l].Datasource[0].Parameter[0]._ = loc + result.Map.Layer[l].Datasource[0].Parameter[0]._;
        }
...
tm.updateResourceFilepath = function(str, loc, callback) {
    tm.parser.parseString(str, function (err, result) {
        if (err) return callback(err);
        var builder = new xml2js.Builder();
        var xml = builder.buildObject(result);
